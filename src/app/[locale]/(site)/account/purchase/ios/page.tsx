...
export default function Page() {
  ...
  return (
    <>
      ...
      />
      {rgInterface ? (
        <IOSProductList rgInterface={rgInterface} />
      ) : (
        <div>Loading</div>
      )}
    </>
...
function IOSProductList({ rgInterface }: { rgInterface: any }) {
  const ITEMS = [
    {
      id: 'rg.1m.nonsub',
      label: '1개월 이용권',
    ...
    rgInterface.subscribe(
      ...
      (returnModel: {
        ...
        if (returnModel.result) {
          alert('model: ' + `${JSON.stringify(returnModel)}`)
        }
      },
    )
  }
